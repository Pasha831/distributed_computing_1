#ifndef PEPLINLABS_RWLOCK_READERS_H
#define PEPLINLABS_RWLOCK_READERS_H
...
int rwlock_wrlock(rwlock_type *rwlock) {
    ...
    while (rwlock->readers_counter > 0 || rwlock->writing_block_wait > 0 || rwlock->is_writer_blocked) {
...
int rwlock_unlock(rwlock_type *rwlock) {
    ...
    if (rwlock->readers_counter > 0) {
        rwlock->readers_counter--;
        if (rwlock->readers_counter == 0 && rwlock->writing_block_wait > 0) {
